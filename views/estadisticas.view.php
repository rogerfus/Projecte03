...
// SELECT * 
// FROM  `tbl_reservas` 
// JOIN  `tbl_recursos` ON  `tbl_reservas`.rec_id =  `tbl_recursos`.rec_id
// JOIN  `tbl_usuarios` ON  `tbl_reservas`.usu_id =  `tbl_usuarios`.usu_id
	...
	//for ($i=4; $i < 19 ; $i++) { 
		//$contador = 0;
		$statement = $conexion->prepare('SELECT * FROM `tbl_reservas` JOIN `tbl_recursos` ON `tbl_reservas`.rec_id=`tbl_recursos`.rec_id JOIN  `tbl_usuarios` ON  `tbl_reservas`.usu_id =  `tbl_usuarios`.usu_id'); 
		//WHERE rec_id = :rec_id');
		...
		<?php
			//echo '<div class="desc">';
			echo '<h4><a href="#"> '.$rec_name.'</a></h4>';
			echo 'Veces reservado: '.$contador .'<br/>';
